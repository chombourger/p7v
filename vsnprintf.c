 ...
 * Usage
 *
 *    The following preprocessor macros should be defined to 1 if the feature or
...
int
trace_vsnprintf(char *str, size_t size, const char *format, va_list args)
{
	LDOUBLE fvalue;
	INTMAX_T value;
...
int
trace_vasprintf(char **ret, const char *format, va_list ap)
{
	size_t size;
	int len;
	va_list aq;
...
#if HAVE_STDARG_H
int
mt_snprintf(char *str, size_t size, const char *format, ...)
#else
int
mt_snprintf(va_alist) va_dcl
#endif	/* HAVE_STDARG_H */
{
#if !HAVE_STDARG_H
	char *str;
...
#if HAVE_STDARG_H
int
mt_asprintf(char **ret, const char *format, ...)
#else
int
mt_asprintf(va_alist) va_dcl
#endif	/* HAVE_STDARG_H */
{
#if !HAVE_STDARG_H
	char **ret;
	...
	VA_SHIFT(ap, format, const char *);
	len = trace_vasprintf(ret, format, ap);
	va_end(ap);
	return len;
}
#endif	/* !HAVE_ASPRINTF */
